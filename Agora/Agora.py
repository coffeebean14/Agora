...
Compile with:
pyinstaller -F -w -i icon.ico --hidden-import tkinter --hidden-import tkinter.ttk --hidden-import ttkthemes --hidden-import pycrypto --add-data 'images/*:images' --add-data 'users.txt:.' Agora.py
'''
...
class MainWindow(Tk):
    def __init__(self, username, host, port, server_pwd):
        Tk.__init__(self)
        self.title(string = "Agora | Welcome, %s" % username)
        self.resizable(0,0)
        #self.style = Style()
        #self.style.theme_use("clam")
        self.ttkStyle = ThemedStyle()
